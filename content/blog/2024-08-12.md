...
GooglePhotosから画像を表示する方法を、共有リンクからコンポーネント内で無理やり画像URLを取得する方法から、シンプルに画像URLを取得してから普通のimgタグで表示するようにした。
...
画像1個貼るたびにAPI通信走ってたのは全然いけてないので、レスポンスをキャッシュしたり `useLazyFetch` を使ったり色々してみたけど最終的にそもそもコンポーネント1つでAPI通信が走る設計になっているのが自分で嫌になってやめた。
